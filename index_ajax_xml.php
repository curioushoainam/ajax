...
<script type="text/javascript" charset="utf-8">
	$('#xml-click').click(function(){	
		...
		$.ajax({
            ...
            dataType : "xml",		// data type of server respose
            ...
            success : function(result){			// call-back function uses for process server response which will store inside variable result            	
	    		...
	    		$(result).find('item').each(function(key, value){
	    			html += '<tr>';
	    				html += '<td>';
	    					html += $(value).find('track_id').text();
	    				html += '</td>';
	    				html += '<td>';
	    					html += $(value).find('title').text();
	    				html += '</td>';
	    			html += '</tr>';
	    		});   			
	   			
	     		$('#xml-result').html(html);
            },
